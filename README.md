...
Wit the following specifications:
 ```bash
...
```
I decided to implement the error returns as generated by printf() on Ubuntu into my project. The function int check_error() checks if an ecountered '%' in the format string is at the EOF or is exclusively followed by space chars and stops printing and returns '-1' if that's the case:
